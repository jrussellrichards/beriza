    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="px-8 py-6 border-b border-line bg-surface shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-title font-semibold text-ink">Dashboard BERISA</h1>
            <p className="text-body text-ink-muted mt-0.5">Visión global de la plataforma</p>
          </div>
          <a
            href="/admin/mandantes"
            className="flex items-center gap-2 text-body text-ink-muted border border-line px-4 py-2 rounded-lg hover:bg-surface-app transition-colors"
          >
            Gestionar mandantes
      ...
      <div className="flex-1 overflow-auto px-8 py-6 space-y-6">
          ...
          {[
            ...
            return (
              <div key={k.label} className="bg-surface rounded-xl border border-line px-5 py-4">
                ...
                <p className={cn("text-metric font-semibold tabular", k.color)}>{k.value}</p>
                <p className="text-meta text-ink-subtle mt-1">{k.sub}</p>
              </div>
            )
          })}
        </div>
        ...
        <div className="grid grid-cols-5 gap-6">
          ...
          <div className="col-span-3 bg-surface rounded-xl border border-line overflow-hidden">
            <div className="px-5 py-4 border-b border-line-subtle flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-strong font-semibold text-ink">Mandantes</h2>
              </div>
              <a href="/admin/mandantes" className="text-meta text-ink-subtle hover:text-ink-muted transition-colors">Ver todos →</a>
            </div>
                  ...
                  <div key={m.id} className="px-5 py-4 flex items-center gap-4 hover:bg-surface-app/60 transition-colors cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-strong font-medium text-ink truncate">{m.razon_social}</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-meta text-ink-subtle">{m.acreditadas}/{m.total_contratistas} acreditados</span>
                      </div>
                    </div>
          ...
          <div className="col-span-2 bg-surface rounded-xl border border-line p-6 flex flex-col">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-strong font-semibold text-ink">Acreditación global</h2>
            </div>
            ...
            <div className="flex items-center justify-center flex-1 py-2">
              <div className="relative w-36 h-36">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <p className="text-metric font-semibold text-ink">{pct}%</p>
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              <div className="rounded-lg bg-surface-app border border-line-subtle px-3 py-2.5 text-center">
                <p className="text-title font-semibold text-ok-ink">{totalAcreditadas}</p>
                <p className="text-[10px] text-ink-subtle">Acreditados</p>
              </div>
              <div className="rounded-lg bg-surface-app border border-line-subtle px-3 py-2.5 text-center">
                <p className="text-title font-semibold text-ink-muted">{totalContratistas - totalAcreditadas}</p>
        ...
        <div className="bg-surface rounded-xl border border-line overflow-hidden">
          <div className="px-5 py-4 border-b border-line-subtle flex items-center gap-2">
            ...
            <h2 className="text-strong font-semibold text-ink">Actividad reciente — todos los mandantes</h2>
          </div>
          <div className="divide-y divide-line">
            {actividad.map((a, i) => {
              ...
              return (
                <div key={i} className="px-5 py-3.5 flex items-center gap-4 hover:bg-surface-app/60 transition-colors">
                  ...
                  <span className="text-micro font-semibold text-ink-subtle w-32 shrink-0">{a.mandante}</span>
                  <p className="text-body text-ink-secondary flex-1">{a.accion}</p>
                  <p className="text-meta text-ink-subtle shrink-0">{a.tiempo}</p>
                </div>
              )
            })}
          </div>
