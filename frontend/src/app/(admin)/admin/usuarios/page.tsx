  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-6 py-5 border-b border-line-subtle flex items-center justify-between">
        <div>
          <p className="text-strong font-semibold text-ink">Nuevo usuario</p>
          <p className="text-meta text-ink-subtle mt-0.5">Se enviará invitación por email</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        <div className="space-y-1.5">
          <label className="text-strong font-medium text-ink-secondary">Nombre completo</label>
          <input
        ...
        <div className="space-y-1.5">
          <label className="text-strong font-medium text-ink-secondary">Rol</label>
    ...
    <div className="flex h-screen overflow-hidden">
      <div className={cn("flex-1 flex flex-col min-w-0 transition-all duration-300", creando ? "mr-96" : "")}>
        ...
        <div className="px-8 py-6 border-b border-line bg-surface shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-title font-semibold text-ink">Usuarios</h1>
              <p className="text-body text-ink-muted mt-0.5">Todos los usuarios registrados en la plataforma</p>
            </div>
            <button
        ...
        <div className="flex-1 overflow-auto px-8 py-6 space-y-5">
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1 max-w-xs">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-body border border-line rounded-lg bg-surface text-ink placeholder:text-ink-subtle focus:outline-none focus:ring-2 focus:ring-brand/20"
              />
            </div>
          ...
          <div className="bg-surface border border-line rounded-xl overflow-x-auto">
            <table className="w-full text-body">
              <thead>
                <tr className="border-b border-line-subtle bg-surface-app/60">
                  <th className="text-left px-5 py-3 text-micro font-semibold text-ink-muted uppercase tracking-wider">Usuario</th>
                  <th className="text-left px-4 py-3 text-micro font-semibold text-ink-muted uppercase tracking-wider">Rol</th>
                  <th className="text-left px-4 py-3 text-micro font-semibold text-ink-muted uppercase tracking-wider">Mandante</th>
                  <th className="text-left px-4 py-3 text-micro font-semibold text-ink-muted uppercase tracking-wider">Estado</th>
                  <th className="text-left px-4 py-3 text-micro font-semibold text-ink-muted uppercase tracking-wider">Creado</th>
              ...
              <tbody className="divide-y divide-line">
                {filtrados.map(u => (
                  <tr key={u.id} className="hover:bg-surface-app/70 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-meta text-ink-subtle">{u.email}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 text-body text-ink-muted">{u.mandante ?? "—"}</td>
                    <td className="px-4 py-4">
                      <span className={cn("inline-flex items-center gap-1.5 text-micro font-medium", u.activo ? "text-ok-ink" : "text-ink-subtle")}>
                    ...
                    <td className="px-4 py-4 text-meta text-ink-subtle">{u.ultimo_acceso}</td>
                    <td className="px-4 py-4">
                      ...
                      {!u.es_uno_mismo && (
                        <button
                          onClick={() => setCuenta(u)}
                          className="text-meta text-ink-muted hover:text-ink px-2 py-1 rounded hover:bg-surface-sunken transition-colors"
                        >
                          Editar
                        </button>
                      )}
            ...
            <div className="px-5 py-3 border-t border-line-subtle bg-surface-app/50">
              <p className="text-meta text-ink-subtle">{filtrados.length} usuario{filtrados.length !== 1 ? "s" : ""}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {(aviso || errorAccion) && (
        <div className="px-6 sm:px-8 pb-4">
          <p className={cn(
            "text-body px-4 py-2 rounded-lg border",
            errorAccion
